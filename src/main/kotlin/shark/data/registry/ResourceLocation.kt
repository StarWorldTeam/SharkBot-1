...
typealias ResourceLocationMap <T> = Map<ResourceLocation, T>
typealias ResourceLocationBiMap <T> = BiMap <ResourceLocation, T>
...
class ResourceLocation private constructor(val namespace: String, val path: String) {
    ...
    companion object {
        ...
        fun isValidPath(path: String): Boolean {
            return path.chars().allMatch { i: Int -> isPathCharacter(i.toChar()) }
        }

        fun isValidNamespace(namespace: String): Boolean {
            return namespace.chars().allMatch { i: Int -> isNamespaceCharacter(i.toChar()) }
        }
    ...
    override fun toString() = "%s:%s".format(Locale.ROOT, namespace, path)
